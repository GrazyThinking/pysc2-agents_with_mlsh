...
flags.DEFINE_integer("warmup_length", 64, "Length of warmup period.")
flags.DEFINE_integer("joint_update_length", 64, "Length of joint update period.")
...
def _main(unused_argv):
    ...
    if agent_name == 'A3CAgent':
        ...
        agent = agent_cls(FLAGS.training, FLAGS.minimap_resolution, FLAGS.screen_resolution)
        agent.build_model(False, 0, FLAGS.net)
        config = tf.ConfigProto(allow_soft_placement=True)
        config.gpu_options.allow_growth = True
        sess = tf.Session(config=config)
        ...
        with sc2_env.SC2Env(
            ...
            env = available_actions_printer.AvailableActionsPrinter(env)
            for x in range(MAX_AGENT_STEPS):
                agent.reset_master_policy()
                agent.sample_env(env)
                mini_ep = warmup_time + joint_update_time
                for i in range(mini_ep):
                    for w in range(warmup_time):
                        replay_buffer = agent.collect_exp()
                        agent.update_master_policy(replay_buffer)
                    for u in range(joint_update_time):
                        replay_buffer = agent.collect_exp()
                        agent.update_master_policy(replay_buffer)
                        agent.update_subpolicies(replay_buffer)
                if i % FLAGS.snapshot_step == 1:
                    agent.save_model(SNAPSHOT, i)
